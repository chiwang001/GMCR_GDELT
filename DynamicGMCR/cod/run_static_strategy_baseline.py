#!/usr/bin/env python3
"""Static strategy-priority baseline for the DynamicGMCR GMCR state graph.

The baseline keeps the DynamicGMCR states, options, and transition graph fixed, but
replaces dynamic preference updating with fixed, lexicographically ordered
strategy-priority declarations for CN and US. It reports GMCR stability and the
static model's ability to explain the historical state sequence.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run the self-contained static strategy-priority GMCR baseline for DynamicGMCR."
    )
    parser.add_argument("--input-dir", type=Path, default=project / "input")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project / "output" / "static_preference_model",
    )
...
def preference_margin(
    ...
    """Return the strategy-priority utility difference between two states."""
    return float(preferences[actor, better] - preferences[actor, worse])
...
def is_preferred(
    ...
    return preference_margin(preferences, actor, better, worse, tolerance) > tolerance
...
def player_stability(
    ...
    ui = np.array(
        [move for move in moves if is_preferred(utilities, player, int(move), state, tolerance)],
        dtype=int,
    )
    if len(ui) == 0:
    ...
    for improved in ui:
        ...
        harmful = np.array(
            [
                ...
                if not is_preferred(utilities, player, int(sanction), state, tolerance)
            ],
            dtype=int,
        )
        if len(harmful) == 0:
            ...
            for sanctioned in harmful:
                ...
                if not any(
                    is_preferred(utilities, player, int(counter), state, tolerance)
        ...
        opponent_improvements = np.array(
            [
                sanction
                for sanction in sanctions
                if is_preferred(
                    utilities, opponent, int(sanction), int(improved), tolerance
                )
            ],
            dtype=int,
        )
        seq_sanctions = np.array(
            [
                sanction
                for sanction in opponent_improvements
                if not is_preferred(utilities, player, int(sanction), state, tolerance)
            ],
            dtype=int,
        )
...
def explain_history(
    ...
    summary = {
        ...
        "interpretation": (
            "Static preferences are generated from fixed strategy-priority declarations. "
            "Binary powers preserve their lexicographic order; no monthly event shocks, "
            "dynamic preference states, estimated option weights, or optimized preference "
            "parameters are used."
        ),
    }
...
def write_outputs(
    ...
    priority_rows = []
    priority_counts = {
        country: len([item for item in priorities if item.country == country])
        for country in COUNTRIES
    }
        ...
        priority_rows.append(
            {
                ...
                "lexicographic_weight": int(
                    2 ** (priority_counts[priority.country] - priority.priority)
                ),
...
def run_baseline(config: BaselineConfig) -> dict[str, Any]:
    ...
    history, summary = explain_history(
        data,
        utilities,
        flags,
        config,
